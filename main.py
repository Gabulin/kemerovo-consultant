...
KB_FILES = [
    ...
    "kb/04_organizations.md", "kb/05_documents.md", "kb/06_faq.md",
]
...
MEASURES_LOWINCOME = [
    ("Единое пособие на детей", "до 17 лет, 50/75/100% ПМ ребёнка (16 717 ₽ в 2026 г.). СФР / Госуслуги / МФЦ"),
...
def classify(message):
    ...
    overview_markers = ("какие льгот", "что положено", "что нам положено", "на что", "все льготы",
                       ...
                       "на что можем рассчитыв", "на что рассчитыв", "что дают многодетн",
                       "что положено многодетн", "чем могут помочь", "чем помогут", "что мы можем получ",
                       "что можно получить", "что нам полож", "с чего начать")
...
# ВОЗРАСТ ДЕТЕЙ — ПОДБИРАЙ МЕРЫ ПОД НЕГО
- Если в профиле есть возраст детей — используй его, чтобы не вываливать лишнее, а назвать то, что реально актуально семье. Это главное преимущество перед сухим списком.
- Ребёнок до 3 лет → молочная кухня, пособие по уходу до 1,5 лет, компенсация за ясли.
- Ребёнок 3–6 лет → детсад (компенсация платы 20/50/70%, первоочередной приём), бесплатные лекарства детям до 6 лет (малоимущим).
- Ребёнок 7–17 лет (школьник) → бесплатное питание в школе (малоимущим), выплата на школьную форму 12 000 ₽, бесплатный проезд школьника.
- Ребёнок 18–23 лет → меры действуют, только если он учится ОЧНО. Если форма обучения неизвестна — коротко уточни её, прежде чем обещать выплату или статус.
- Не предлагай детсад семье со взрослыми детьми и не предлагай вуз семье с малышами — это и есть «нелогичные ответы», которых нужно избегать.

# НЕ ПЕРЕСПРАШИВАЙ ОЧЕВИДНОЕ
- Не спрашивай то, что уже есть в профиле (число детей, возраст, состав, доход, район, ипотека). Уточняй только реально недостающее для точного ответа (например, очную форму обучения совершеннолетнего ребёнка или гражданство).
...
# ФОРМАТ ОТВЕТА (чтобы легко читалось)
- На вопрос «как оформить / какие документы / куда подать» давай порядок действий нумерованными пунктами (1. 2. 3.) — коротко, по одному шагу на строку.
- Перечисления мер/документов давай маркированным списком (каждый пункт с новой строки, начиная с «• » или «- »).
- Суммы, проценты, сроки, даты и названия мер выделяй **жирным** — так пользователю проще выхватить главное.
- Между смысловыми блоками оставляй пустую строку. Не пиши «простыню» сплошным текстом и не злоупотребляй списками там, где хватит пары предложений.

Отвечай на русском. Тон — тёплый, профессиональный, без формализма."""
...
def parse_ages(profile):
    """Возвращает список возрастов детей из профиля (0..30)."""
    raw = profile.get("Возраст") or ""
    ages = [int(x) for x in re.findall(r'\d{1,2}', raw)]
    return [a for a in ages if 0 <= a <= 30][:20]


def age_hint(profile):
    """
    Детерминированный «возрастной фокус»: по возрасту детей подсказываем модели,
    какие меры реально актуальны. Работает, даже если модель проигнорировала
    инструкцию — нужная подсказка уже лежит в контексте.
    """
    ...
    if any(a <= 2 for a in ages):
        tips.append("есть ребёнок до 3 лет — актуальны молочная кухня, пособие по уходу до 1,5 лет, компенсация за ясли")
    if any(3 <= a <= 6 for a in ages):
        tips.append("есть дошкольник (3–6) — компенсация платы за детсад (20/50/70%), первоочередной приём, лекарства детям до 6 лет (малоимущим)")
    if any(7 <= a <= 17 for a in ages):
        tips.append("есть школьник (7–17) — выплата на школьную форму 12 000 ₽, бесплатный проезд школьника, питание в школе (малоимущим)")
    if any(a >= 18 for a in ages):
        tips.append("есть совершеннолетний ребёнок (18+) — меры действуют ТОЛЬКО при очной учёбе; если форма обучения неизвестна, уточни её")
    if not tips:
        return ""
    return ("ВОЗРАСТНОЙ ФОКУС (по профилю — предлагай актуальное возрасту, "
            "не предлагай нерелевантное): " + "; ".join(tips) + ".")
...
MAX_MSG_LEN = 1500  # длиннее — обрезаем (защита от мусора и расхода токенов)

@app.post("/api/chat")
async def chat(req: ChatRequest):
    # Нормализуем ввод: пустое сообщение — мягко просим вопрос; очень длинное — обрезаем.
    req.message = (req.message or "").strip()[:MAX_MSG_LEN]
    if not req.message:
        return {"answer": "Напишите, пожалуйста, ваш вопрос — например, «что положено нашей семье» "
                          "или «как оформить компенсацию на ЖКУ»."}
    ...
    if kind == "greeting":
        ...
        ah = age_hint(req.profile)
        context = (registry +
                   (("\n\n" + ah) if ah else "") +
                   ("\n\nДЕТАЛИ ИЗ БАЗЫ:\n" + "\n---\n".join(rag) if rag else "") +
                   "\n\n(Это полный реестр. Не вываливай всё списком — выдели 4-6 самых важных и ценных мер для этой семьи с учётом возраста детей, сгруппируй понятно, предложи углубиться в любую. Начни с денежных выплат.)")
        n_chunks = len(rag)
    else:  # specific
