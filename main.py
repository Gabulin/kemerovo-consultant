...
DOMAIN_ANCHORS = {
    ...
    "ясли", "инвалид", "овз", "опек", "приёмн", "приемн", "усынов", "подопечн",
    "награ", "слав", "герой", "оздоров", "лагер", "путёвк", "путевк", "санатор",
    "очно", "очног", "очередь", "развод", "алимент", "переезд", "прописк", "регистрац",
    "200 000", "200000", "450", "550", "1200", "1 200", "удостовер", "стаж", "ипк",
}
...
# ── Продолжение диалога ──
# Короткие реплики («да», «расскажи подробнее», «а первое?») сами по себе
# не проходят тематический якорь, но в живом диалоге это ПРОДОЛЖЕНИЕ темы.
# Тема берётся из истории: если последний обмен был про льготы — короткая
# реплика тоже про льготы. Это чинит сценарий «бот предложил рассказать
# подробнее → пользователь ответил "Да" → бот отказал как "вне темы"».
FOLLOWUP_MAX_LEN = 90  # реплика длиннее — самостоятельный вопрос, якорь решает сам

# Маркеры продолжения: согласие, просьба раскрыть, отсылка к пункту/предмету.
# Без маркера короткая реплика («рецепт борща») — новая тема, якорь решает сам.
FOLLOWUP_CUE_RE = re.compile(
    r'\b(да|нет|не\s+(надо|нужно|хочу)|давай(те)?|конечно|хочу|можно|ага|угу|ок|окей|расскаж\w*|подробн\w*|'
    r'дальше|продолж\w*|ещё|еще|больше|уточн\w*|поясн\w*|объясн\w*|'
    r'перв\w*|втор\w*|треть\w*|четверт\w*|пят\w*|последн\w*|пункт\w*|'
    r'эт(о|а|у|и|ой|ом|им|их)|неё|нее|ней|него|ним|них|туда|там|оба|обе|все)\b',
    re.I)

def history_on_topic(history):
    """Был ли последний содержательный обмен диалога в теме льгот."""
    for pair in reversed(history):
        u = pair.get("user") or ""
        b = pair.get("bot") or ""
        if u.strip() or b.strip():
            return is_on_topic(u) or is_on_topic(b)
    return False

def is_followup(message, history):
    """Короткая реплика с маркером продолжения при «тематической» истории."""
    return (bool(history)
            and len(message) <= FOLLOWUP_MAX_LEN
            and FOLLOWUP_CUE_RE.search(message) is not None
            and history_on_topic(history))

def followup_query(message, history):
    """Дополняет короткую реплику текстом последнего обмена — чтобы TF-IDF
    искал по теме диалога, а не по словам «да, расскажи подробнее»."""
    parts = [message]
    if history:
        last = history[-1]
        parts.append(last.get("user") or "")
...
MEASURES_UNIVERSAL = [
    ...
    ("Выплата на школьную форму 12 000 ₽", "на каждого школьника (с 2025 — и студента СПО/вуза) ежегодно, приходит автоматически без заявления; если не пришла — отдел соцвыплат / МФЦ"),
...
# Порог у этой меры выше ПМ (1,5 ПМ трудоспособного), поэтому она НЕ в списке
# «для малоимущих»: семья с доходом выше ПМ всё ещё может иметь на неё право.
MEASURES_MIDINCOME = [
    ("Семейная налоговая выплата", "ежегодный возврат части НДФЛ, приём заявлений 1 июня — 30 сентября. "
     "Порог дохода — 1,5 ПМ трудоспособного (≈ 28 178 ₽/чел. в 2026 г.), то есть доступна и части семей "
     "с доходом выше обычного ПМ. СФР / Госуслуги / МФЦ"),
]
...
def build_registry(income_known_low=None):
    ...
    income_known_low:
      ...
      False → доход выше ПМ: без мер для малоимущих, но С семейной налоговой
              выплатой (её порог 1,5 ПМ — выше обычного ПМ)
    ...
    lines.append("\nМЕРА ПРИ ДОХОДЕ ДО 1,5 ПМ ТРУДОСПОСОБНОГО (≈ 28 178 ₽/чел.) — "
                 "предлагай даже при доходе выше обычного ПМ, мягко уточнив уровень дохода:")
    for name, desc in MEASURES_MIDINCOME:
        lines.append(f"• {name} — {desc}")

    if income_known_low is False:
        lines.append("\n(Семья указала доход выше ПМ — меры для малоимущих НЕ показываем и не упоминаем. "
                     "Семейную налоговую выплату упомянуть МОЖНО — у неё порог выше.)")
    else:
...
SYSTEM = """Ты — консультант центра социальной поддержки многодетных семей г. Кемерово. Ты опытный специалист по социальной работе: говоришь живым человеческим языком, по делу, с заботой, но без воды. У тебя нет личного имени — представляйся просто «консультант по льготам», имя не придумывай.

# ПРИВЕТСТВИЕ И ПОВТОРЫ (критично!)
- Здоровайся и коротко представляйся («Здравствуйте! Я консультант по льготам для многодетных семей…») ТОЛЬКО в самом первом сообщении диалога, когда история пуста. Имя НЕ называй — у тебя его нет. Если в истории уже есть твои реплики — НЕ здоровайся и НЕ представляйся заново, сразу отвечай по существу вопроса.
...
# ПРОДОЛЖЕНИЕ ДИАЛОГА (критично!)
- Ты ведёшь ЕДИНЫЙ диалог: история сообщений выше — его часть, всегда учитывай её.
- Короткая реплика пользователя («да», «давай», «хочу», «расскажи подробнее», «а первое?», «и что дальше?») — это ПРОДОЛЖЕНИЕ предыдущей темы, а не новый вопрос.
- Если в прошлом ответе ты предложил рассказать подробнее или задал вопрос, а пользователь согласился («да», «хочу») — раскрой именно то, что предлагал: пошагово, с документами, адресами и сроками. НИКОГДА не отвечай на согласие фразой «я консультирую по льготам, какая тема вас интересует» — тема уже названа в твоей последней реплике.
- Если пользователь ссылается на пункт твоего списка («про второе», «первый вариант», «последнее») — найди этот пункт в своём последнем ответе и раскрой его.
- Местоимения и отсылки («она», «эта выплата», «туда», «для неё») относи к последней обсуждавшейся мере или учреждению из истории.
- Не повторяй в продолжении то, что уже сказал, — добавляй новое.
...
async def chat(req: ChatRequest):
    ...
    history = [p for p in req.history if isinstance(p, dict)]
    followup = is_followup(req.message, history)
    # Для коротких реплик ищем по теме диалога, а не по «да, расскажи»
    search_q = followup_query(req.message, history) if followup else req.message
    ...
    if kind == "greeting":
        ...
        if not is_on_topic(req.message) and not followup:
            ...
            if not rag:
                if followup:
                    context = ("(Пользователь продолжает предыдущую тему диалога. Продолжи её по истории "
                               "сообщений; если конкретных данных не хватает — честно скажи и направь на "
                               "горячую линию СФР 8-800-100-00-01 или dsznko.ru. Не выдумывай.)")
                else:
                ...
                context = "\n\n---\n\n".join(rag)
                if followup and not is_on_topic(req.message):
                    context = ("(ПРОДОЛЖЕНИЕ ДИАЛОГА: реплика пользователя — ответ на твоё предыдущее "
                               "сообщение или просьба раскрыть его подробнее. Посмотри свою последнюю "
                               "реплику в истории и продолжи именно её тему, используя контекст ниже.)"
                               "\n\n---\n\n" + context)
            n_chunks = len(rag)
    ...
    on_domain = kind == "overview" or (
        kind == "specific" and (is_on_topic(req.message) or followup))
    if on_domain:
        # Для продолжений диалога тему определяем по расширенному запросу
        ql = search_q.lower()
        wants_office = kind == "overview" or any(t in ql for t in REGIONAL_OFFICE_TOPICS)
        cb = district_contact_block(req.profile) if (wants_office and n_chunks > 0) else ""
        if cb:
        ...
        # Не гейтим по n_chunks: именно при пустой выдаче страховка нужнее всего.
        hf = hard_facts_block(search_q)
        if hf:
            context = hf + "\n\n---\n\n" + context
